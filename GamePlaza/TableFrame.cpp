...
struct tagTable100ResourceInfo
{
	COLORREF							crName;								//名字颜色
	COLORREF							crNote;								//备注颜色
	COLORREF							crTableID;							//号码颜色
	...
	RECT								rcNote;								//备注位置
	RECT								rcTableID;							//桌号位置
	RECT								rcName[100];					//名字位置
	RECT								rcChair[100];					//椅子位置
	POINT								ptReady[100];					//准备位置
	INT									nDrawStyle[100];				//输出格式
};
...
//配置文件结构
struct tagTable100ConfigFile
{
	WORD								wFileSize;							//文件大小
	WORD								wStructVer;							//结构版本
	tagTable100ResourceInfo				TableResourceInfo;					//配置参数
};
...
bool CTableResource::LoadFromFiles(LPCTSTR pszGameName)
{
	try
	{
		...
		if (lstrcmp(pszGameName,TEXT("DZShowHand"))==0)
		{
			...
			m_ImagePeople.SetLoadInfo(szResourcePath);
		...
		{
			m_ImageLock.SetLoadInfo(szResourcePath,true);
		}
		if (m_ImageLock.IsNull()) 
		{
		...
		m_ImageReady.SetLoadInfo(szResourcePath,true);
		...
		m_ImagePlay.SetLoadInfo(szResourcePath,true);
		...
		m_ImageTable.SetLoadInfo(szResourcePath,true);
		...
		//设置参数
		tagTable100ResourceInfo * pResourceInfo=&TableConfig.TableResourceInfo;
		...
		for (WORD i=100;i<500;i++)
		{
			m_ptReady[i]=CPoint(0,0);
			m_rcName[i]=CRect(0,0,0,0);
			m_rcChair[i]=CRect(0,0,0,0);
			m_nDrawStyle[i]=pResourceInfo->nDrawStyle[0];
		}
		return true;
	}
...
//接口查询
void * __cdecl CTableView::QueryInterface(const IID & Guid, uint32 dwQueryVer)
{
...
//设置用户信息
bool __cdecl CTableView::SetUserInfo(uint32 wChairID, IUserItem * pIUserItem)
{
...
//设置盘旋位置
void __cdecl CTableView::SetHoverChairID(uint32 wHoverChairID)
{
...
//获取用户信息
IUserItem * __cdecl CTableView::GetUserInfo(uint32 wChairID)
{
...
//获取空椅子数
WORD __cdecl CTableView::GetNullChairCount(uint32 & wNullChairID)
{
	WORD wNullCount=0;
...
void __cdecl CTableView::DrawTable(CDC * pDC, bool bHideInfo, CTableResource * pTableResource, bool bShowUser)
{
	...
	for (WORD i=0;i<m_TableState.wChairCount;i++)
	{
		...
		if ((bHideInfo==false)&&(bShowUser==true)&&(m_TableState.pIUserItem[i]!=NULL))
		{
			//变量定义
			CRect rcDrawRect=pTableResource->m_rcName[i];
			tagUserData2 * pUserData=m_TableState.pIUserItem[i]->GetUserData();
			...
			else
			{
				pDC->FillSolidRect(&pTableResource->m_rcChair[i],crBackGround);
				pIUserFaceRes->DrawNormalFace(pDC,pTableResource->m_rcChair[i].left+1,
					pTableResource->m_rcChair[i].top+1,pUserData->wFaceID,pUserData->m_uin,pUserData->dwCustomFaceVer);
			}
			...
			int nDrawStyte=pTableResource->m_nDrawStyle[i];
			int nDrawFormat=DT_WORDBREAK|DT_EDITCONTROL|DT_END_ELLIPSIS;
			int nNameLength=pUserData->szNick.length();
			int nNameHeight=pDC->DrawText(pUserData->szNick.c_str(),nNameLength,&rcDrawRect,nDrawFormat|DT_CALCRECT);
			...
			//用户名字
			pDC->SetTextColor(pTableResource->m_crName);
			pDC->DrawText(pUserData->szNick.c_str(),nNameLength,&rcDrawRect,nDrawFormat);
		}
...
//接口查询
void * __cdecl CTableFrame::QueryInterface(const IID & Guid, uint32 dwQueryVer)
{
...
//配置函数
bool __cdecl CTableFrame::InitTableFrame(WORD wTableCount, WORD wChairCount, bool bHideInfo, const CGameServer * pGameKind, IUnknownEx * pIUnknownEx)
{
	//效验参数
	...
	//获取资源
	TCHAR szDirectory[MAX_PATH]=TEXT("resource\\gameview");
...
//闪动椅子
bool __cdecl CTableFrame::FlashChair(WORD wTableID, uint32 wChairID)
{
	//获取桌子
	ITableView * pITableView=GetTableArrayPtr(wTableID);
	if (pITableView==NULL) return false;
...
//设置用户信息
bool __cdecl CTableFrame::SetUserInfo(WORD wTableID, uint32 wChairID, IUserItem * pIUserItem)
{
	ITableView * pITableView=GetTableArrayPtr(wTableID);
...
//获取用户信息
IUserItem * __cdecl CTableFrame::GetUserInfo(WORD wTableID, uint32 wChairID)
{
	ITableView * pITableView=GetTableArrayPtr(wTableID);
	ASSERT(pITableView!=NULL);
...
//获取空椅子数
WORD __cdecl CTableFrame::GetNullChairCount(WORD wTableID, uint32 & wNullChairID)
{
	ITableView * pITableView=GetTableArrayPtr(wTableID);
	ASSERT(pITableView!=NULL);
...
//资源目录
void CTableFrame::GetGameResDirectory(const CGameServer * pServer, TCHAR szResDirectory[], WORD wBufferCount)
{
	//变量定义
	WORD wStringIndex=0;
	...
	lstrcpyn(szResDirectory,(LPCTSTR)pServer->m_processName.c_str(),wBufferCount);
